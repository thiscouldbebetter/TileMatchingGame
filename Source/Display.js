...
class Display {
    ...
    drawLevel(level) {
        ...
        for (var i = 0; i < tiles.length; i++) {
            var tile = tiles[i];
            this.drawTile(tile, levelMap);
        }
        var cursor = level.cursor;
        this.drawPos.overwriteWith(cursor.pos).add(new Coords(.25, .25)).multiply(mapCellSizeInPixels);
        ...
        g.strokeRect(this.drawPos.x, this.drawPos.y, cursorSizeInPixels.x, cursorSizeInPixels.y);
        var fontHeight = 10; // hack
        g.fillStyle = this.colorBorderAndText;
        g.fillText("Matches:" + level.numberOfMatchesSoFar + "/" + level.numberOfMatchesToWin, 2, levelMap.sizeInPixels.y + fontHeight);
        g.fillText("Moves:" + level.numberOfMovesSoFar + "/" + level.numberOfMovesAllowed, 2, levelMap.sizeInPixels.y + (fontHeight * 2));
    }
    drawMap(map) {
        ...
        for (var y = 0; y < mapSizeInCells.y; y++) {
            ...
            for (var x = 0; x < mapSizeInCells.x; x++) {
                ...
                g.strokeRect(drawPos.x, drawPos.y, cellSizeInPixels.x, cellSizeInPixels.y);
            }
        }
    }
    drawTile(tile, map) {
        ...
        var drawPos = this.drawPos.overwriteWith(tilePos).multiply(mapCellSizeInPixels);
        var drawSize = this.drawSize.overwriteWith(mapCellSizeInPixels);
        ...
        if (this.useColors) {
            ...
            if (tileFractionOfLifeRemaining == null) {
                g.fillStyle = this.colorBorderAndText;
                g.fillText(tileDefn.symbol, drawPos.x + (mapCellSizeInPixels.x - tileWidth) / 2, drawPos.y + mapCellSizeInPixels.y * .6);
            }
            else {
                g.fillStyle = "Black";
                g.fillText(tileDefn.symbol, drawPos.x + (mapCellSizeInPixels.x - tileWidth) / 2, drawPos.y + mapCellSizeInPixels.y * .6);
    ...
    initialize(useColors, viewSizeInPixels) {
